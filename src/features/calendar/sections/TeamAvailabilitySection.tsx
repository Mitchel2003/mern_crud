    ...
    <section className={cn('p-4 rounded-lg shadow-sm border', theme === 'dark' ? 'bg-zinc-950 border-zinc-700' : 'bg-white border-gray-100')}>
      ...
      <div className="space-y-3">
        {teamMembers.length > 0
          ? teamMembers.map(member => <TeamMemberCard theme={theme} key={member.id} member={member} />)
          : <p className={cn('text-sm py-3 text-center', theme === 'dark' ? 'text-gray-400' : 'text-gray-500')}> No hay miembros del equipo disponibles </p>}
      </div>
    </section>
  )
}
...
function TeamMemberCard({ member, theme }: TeamMemberCardProps) {
  const status = availabilityStatus(theme)[member.availability]
  return (
    <div className={cn('p-3 rounded-lg border flex items-center', theme === 'dark'
      ...
      <div className="relative">
        ...
        <span className={cn('absolute bottom-0 right-0 w-3 h-3 rounded-full border-2',
          theme === 'dark' ? 'border-zinc-900' : 'border-white', status.color
        )}></span>
      </div>
...
/*--------------------------------------------------tools--------------------------------------------------*/
/** Obtiene el estado de disponibilidad de un miembro del equipo */
const availabilityStatus = (theme: string) => ({
  available: {
    label: 'Disponible',
    color: theme === 'dark'
